...
const DOM = {
  ...
  renderAppLayout: function () {
    ...
    root.innerHTML =
      '<header class="' +
      CONFIG.DOM_IDS.TOP_BAR +
      '"></header>' +
      '<main class="' +
      CONFIG.DOM_CLASSES.APP_LAYOUT +
      '">' +
      '<aside class="' +
      CONFIG.DOM_IDS.SIDEBAR +
      '"></aside>' +
      '<section class="' +
      CONFIG.DOM_IDS.WORKSPACE +
      '"></section>' +
      '<aside class="' +
      CONFIG.DOM_IDS.STAGE +
      '"></aside>' +
      "</main>";
  },
  ...
  updateUIFromState: function () {
    ...
    if (topBar && topBar.componentInstance) {
      topBar.componentInstance.updateLevel(
        gameState.getCurrentLevel(),
        gameState.getTotalLevels(),
      );
      topBar.componentInstance.updateStars(
  ...
  clearWorkspaceVisual: function () {
    ...
    const placeholder = workspace.querySelector(
      "." + CONFIG.DOM_CLASSES.WORKSPACE_PLACEHOLDER,
    );
    if (placeholder) {
      placeholder.style.display = "none";
    }

    const stacks = workspace.querySelectorAll(
      "." + CONFIG.DOM_CLASSES.BLOCK_STACK,
    );
